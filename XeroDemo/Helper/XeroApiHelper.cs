...
namespace XeroDemo.Helpers
{
    ...
    public static class XeroApiHelper
    {
        ...
        static XeroApiHelper()
        {
            ...
            var callbackUrl = Common.Helper.CallBackUrl;//  ConfigurationManager.AppSettings["callbackUrl"];
            var memoryStore = new MemoryTokenStore();
            var requestTokenStore = new MemoryTokenStore();
            var baseApiUrl = Common.Helper.baseApiUrl;// ConfigurationManager.AppSettings["baseApiUrl"];

            // Consumer details for Application
            var consumerKey = Common.Helper.ClientId; // ConfigurationManager.AppSettings["consumerKey"];
            var consumerSecret = Common.Helper.client_secret;// ConfigurationManager.AppSettings["consumerSecret"];
